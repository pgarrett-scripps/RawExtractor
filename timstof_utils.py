...
def generate_ms2(tdf_conn):
    ...
    for row in precursor_list:
        ...
        if monoisotopic_mz is not None and cs is not None:
            ...
            if len(mz_arr) > 0:
                ms2_lines.append("S\t{0:06d}\t{1:06d}\t{2:.4f}\n".format(scan_id, scan_id, prc_mass_mz))
                ms2_lines.append("I\tTIMSTOF_Parent_ID\t{}\n".format(parent))
                ms2_lines.append("I\tTIMSTOF_Precursor_ID\t{}\n".format(prc_id))
                ms2_lines.append("I\tPrecursor Intensity\t{0:.4f}\n".format(intensity))
                ms2_lines.append("I\tRetTime\t{0:.4f}\n".format(rt_time))
                ms2_lines.append("I\tIon Mobility\t{0:.4f}\n".format(k0[0]))
                ms2_lines.append("I\tCCS\t{0:.4f}\n".format(timsdata.oneOverK0ToCCSforMz(k0[0], cs, prc_mass_mz)))
                ms2_lines.append("I\tCollision Energy\t{0:.2f}\n".format(collision_energy))
                ms2_lines.append("Z\t{1}\t{0:.4f}\n".format(prc_mass, cs))

                int_arr = mz_int_arr[prc_id_int][1]
                for j in range(0, len(mz_arr)):
                    ms2_lines.append("%.4f %.1f \n" % (mz_arr[j], int_arr[j]))
